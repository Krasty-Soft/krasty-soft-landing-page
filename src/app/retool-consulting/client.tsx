...
const whatWeCovers = [
  ...
  <>Handover package for your team or a <strong>Retool development agency</strong>.</>,
...
const whyKrasty = [
  ...
  <>A predictable <strong>Retool consultancy</strong> format with measurable outputs.</>,
  'Experience across FinTech, healthcare, E-commerce, and logistics.',
]

const ContentBlock = ({ heading, children }: { heading: string; children: React.ReactNode }) => (
  <div>
    <h2
...
export default function RetoolConsultingClient({ cases }: RetoolConsultingClientProps) {
  return (
    <>
      {/* Hero Section */}
        ...
        <div style={{ paddingTop: '4rem', paddingBottom: '5rem' }}>
          ...
          >
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              >
                <Lightbulb size={32} color="var(--brand-red)" style={{ display: 'inline' }} />
              </motion.div>
            </div>
            ...
            >
              <p style={bodyTextStyle}>
                Krasty Soft provides <strong>Retool consulting</strong> to help teams address
                fragmented internal workflows, improve data visibility, and keep Retool apps
                stable as requirements change. You get practical guidance, clear priorities,
                and changes that can be shipped fast without rebuilding everything from scratch.
              </p>
              <p style={bodyTextStyle}>
                This format fits product teams, ops teams, and SMBs that need reliable internal
                tooling and <strong>low code development</strong>.
              </p>
            </motion.div>
          </motion.div>
        </div>
          ...
          >
            <ContentBlock heading="When Retool Consulting Is the Right Choice">
              ...
              <ul
                ...
                style={{ ...bodyTextStyle, paddingLeft: '1.5rem', listStyleType: 'disc' }}
              >
            ...
            <ContentBlock heading="What Problems We Solve">
              ...
              >
                ...
                {problemsWesolve.map((item, i) => (
                  <>
                    <div key={`p-${i}`} style={{ padding: '1rem 1.25rem', borderBottom: i < problemsWesolve.length - 1 ? '1px solid var(--border-default)' : 'none', borderRight: '1px solid var(--border-default)', color: 'var(--text-secondary)', fontSize: '0.9375rem', lineHeight: '1.7' }}>{item.problem}</div>
                    <div key={`s-${i}`} style={{ padding: '1rem 1.25rem', borderBottom: i < problemsWesolve.length - 1 ? '1px solid var(--border-default)' : 'none', color: 'var(--text-secondary)', fontSize: '0.9375rem', lineHeight: '1.7' }}>{item.solution}</div>
                  </>
                ))}
              </div>
        ...
        <SectionWrapper>
          ...
          >
            <ContentBlock heading="What Our Retool Consultancy Covers">
              <p style={bodyTextStyle}>Our <strong>Retool consulting</strong> typically covers:</p>
              <ol style={{ ...bodyTextStyle, paddingLeft: '1.5rem', listStyleType: 'decimal' }}>
              ...
              <p style={bodyTextStyle}>
                ...
                <strong>Retool development services</strong> after the consulting phase.
              </p>
            </ContentBlock>
          </motion.div>
        </SectionWrapper>
                ...
                {howWeWork.map((item, i) => (
                  <div key={i} className="flex items-start flex-1">
                    <div className="flex flex-col items-center flex-1">
                      <div style={{ width: '2.5rem', height: '2.5rem', borderRadius: '50%', backgroundColor: 'rgba(220, 38, 38, 0.15)', border: '1px solid rgba(220, 38, 38, 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1rem', color: 'var(--brand-red)', flexShrink: 0, marginBottom: '0.75rem' }}>
                        {i + 1}
                      </div>
                      <p style={{ ...bodyTextStyle, textAlign: 'center', marginBottom: 0, fontSize: '0.9375rem' }}>{item}</p>
                    </div>
                    {i < howWeWork.length - 1 && (
                      <div style={{ marginTop: '1.15rem', height: '1px', width: '2rem', flexShrink: 0, backgroundColor: 'rgba(220, 38, 38, 0.3)' }} />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-4 md:hidden">
                {howWeWork.map((item, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div style={{ width: '2.25rem', height: '2.25rem', borderRadius: '50%', backgroundColor: 'rgba(220, 38, 38, 0.15)', border: '1px solid rgba(220, 38, 38, 0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.9375rem', color: 'var(--brand-red)', flexShrink: 0 }}>
                      {i + 1}
                    </div>
                    <p style={{ ...bodyTextStyle, marginBottom: 0, paddingTop: '0.25rem' }}>{item}</p>
                  </div>
                ))}
              </div>
            </ContentBlock>
        ...
        <SectionWrapper>
          ...
          >
            <ContentBlock heading="Why Krasty Soft">
              ...
              <ul style={{ ...bodyTextStyle, paddingLeft: '1.5rem', listStyleType: 'disc', textAlign: 'left' }}>
              ...
              <p style={{ ...bodyTextStyle, textAlign: 'left' }}>
                Let&apos;s review your Retool setup now: our team is ready to help you.
              </p>
            </ContentBlock>
          </motion.div>
        </SectionWrapper>
      ...
      <Section variant="primary" animate={false}>
        <CTABanner>
          <CheckCircle size={48} color="white" style={{ margin: '0 auto 1.5rem', display: 'block' }} />
