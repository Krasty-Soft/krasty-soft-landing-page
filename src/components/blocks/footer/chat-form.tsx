...
type Step = 'name' | 'email' | 'phone' | 'purpose' | 'message' | 'file' | 'done'
...
export const ChatForm = () => {
    ...
    const handleNextStep = () => {
        ...
        const isSkipCommand = ['no', 'skip', 'nope', 'na', 'n/a', 'pass'].includes(valueLower)

        // Validation for required fields
        if (currentStep === 'name' && value.length < 2) {
            addUserMessage(value)
            addBotMessage("Hmm, that name seems too short. Could you tell me your full name? 😊", 500)
            setInput('')
            return
        }
        ...
        if (currentStep === 'message' && value.length < 10 && !isSkipCommand) {
            addUserMessage(value)
            addBotMessage("Could you tell me a bit more? At least 10 characters please. 📝", 500)
            setInput('')
            return
        }
        ...
        switch (currentStep) {
            ...
            case 'email':
                ...
                addBotMessage('Your phone number? (Type "skip" if you prefer not to share)', 1500)
