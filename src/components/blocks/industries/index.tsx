...
export const Industries = () => {
  return (
    <Section
      variant={"paper"}
      subtitle={"INDUSTRIES"}
      title={"Tailored solutions for leading industries."}
    >
        ...
        {Array.isArray(PAGES.industries) &&
          PAGES.industries.map((item, index) => {
            ...
            return (
              <li
                key={index}
                className="bg-white p-5 lg:p-6 xl:p-8 rounded-[20px]"
              >
                <div className="h-8 w-8 center bg-black rounded-full mb-7 lg:mb-6 xl:mb-8 lg:h-10 lg:w-10 xl:h-c-50 xl:w-c-50">
                  {Icon && <Icon className="lg:scale-150" />}
                </div>
                <h3 className="font-semibold text-base mb-3 lg:mb-2 tracking-wider lg:text-lg xl:text-1xl">
                  {item.label}
                </h3>
                <p className="font-medium text-sm leading-loose mb-8 xl:text-base xl:mb-11">
                  {item.description}
                </p>
